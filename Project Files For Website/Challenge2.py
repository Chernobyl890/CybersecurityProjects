...
initial_value = 0x4F574154
feedback_value = 0xFFFFFFFF # set the feedback value to 0x87654321 (32 bit hex value) which is used in the step function to determine which bits are involved in the feedback op
# 0x87654321
# Read the KDB file path from command line argument
if len(sys.argv) != 2:
    print("Usage: python Challenge2.py <path_to_kdb_file>") # path needs to include "\\" double slashes!!
    sys.exit(1)

kdb_file_path = sys.argv[1]

with open(kdb_file_path, "rb") as kdb_file:
    kdb_data = kdb_file.read()

input_data = bytearray(kdb_data) # initializes the input data with a sequence of bytes (this will be encrypted using the LFSR-generated key stream)
...
input_data1 = bytearray(encrypted_data)
lfsr1 = LFSR(feedback_value, initial_value)

encrypted_data1 = encrypt(input_data1, lfsr1)
print("File Name: ", kdb_file_path)
print("Unencrypted data:", encrypted_data1) 
